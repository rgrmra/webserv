...
bool directive::validateName(string name) {
  ...
  if (name[0] == '-' || name[0] == '.' || name[name.size() - 1] == '-' ||
      name[name.size() - 1] == '.') {
    return false;
  }
...
bool directive::validateHttpMethod(string method) {
  list<string> allowed_methods =
      parser::split(parser::DEFAULT_ALLOW_METHODS, ' ');
...
void directive::addErrorPage(string error_page,
...
void directive::mergeErrorPages(map<string, string> error_pages,
                                map<string, string> &_error_pages) {
  ...
  for (; serverIt != _servers.end(); serverIt++) {
    ...
    for (; exitenListenIt != exitenListen.end(); exitenListenIt++) {
      ...
      for (; newListenIt != newListen.end(); newListenIt++) {
        ...
        if (tmp2.front() != "0.0.0.0" && tmp.front() == tmp2.front() &&
            tmp.back() == tmp2.back()) {

          logger::warning(
              "conflicting server name \"" +
              (server.getNames().size() ? server.getNames()[0] : "") +
