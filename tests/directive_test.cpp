...
TEST(DirectiveTest, SetAcessLog) {
    ...
    EXPECT_THROW(directive::setAcessLog("invalid access log", result), runtime_error);
    ...
    EXPECT_THROW(directive::setAcessLog("  leading_trailing  ", result), runtime_error);
    EXPECT_TRUE(result.empty()); // Ensure result is not modified on error
}
...
TEST(DirectiveTest, SetErrorLog) {
    ...
    EXPECT_THROW(directive::setErrorLog("invalid error log", result), runtime_error);
    EXPECT_TRUE(result.empty()); // Ensure result is not modified on error
    ...
    EXPECT_THROW(directive::setErrorLog("  leading_trailing  ", result), runtime_error);
    EXPECT_TRUE(result.empty()); // Ensure result is not modified on error
}
...
TEST(DirectiveTest, ValidateHttpListen) {
    ...
    EXPECT_FALSE(directive::validateHttpListen("[2001:0db8:85a3:0000:0000:8a2e:0370:7334]:80"));

    // Test with invalid characters
    EXPECT_FALSE(directive::validateHttpListen("127.0.0.1:abc")); // Non-numeric port
    EXPECT_FALSE(directive::validateHttpListen("127.0.0.1:8080a")); // Invalid character at the end
    EXPECT_FALSE(directive::validateHttpListen("127.0.0.1:8080:")); // Extra colon
    EXPECT_FALSE(directive::validateHttpListen("127.0.0.1:8080 ")); // Space at the end
    ...
    EXPECT_FALSE(directive::validateHttpListen("127.0.0.1::8080")); // Double colon

    // Test with more than two parts after splitting by colon
    EXPECT_FALSE(directive::validateHttpListen("127.0.0.1:8080:extra")); // More than two parts

    // Test with valid port-only input
    EXPECT_TRUE(directive::validateHttpListen("8080")); // Port only
    EXPECT_FALSE(directive::validateHttpListen(":8080")); // Port only with leading colon (invalid in your function)
    EXPECT_FALSE(directive::validateHttpListen("8080:")); // Port only with trailing colon

    // Test with invalid IPv6 format
    EXPECT_FALSE(directive::validateHttpListen("[::1]::8080")); // Double colon in IPv6
    EXPECT_FALSE(directive::validateHttpListen("[::1]:8080:extra")); // More than two parts
}
...
TEST(DirectiveTest, ValidateHttpHost) {
    ...
    // Test with invalid characters
    EXPECT_FALSE(directive::validateHttpHost(string("192.168.1.a"))); // Contains non-numeric character
    EXPECT_FALSE(directive::validateHttpHost(string("192.168.1.1 "))); // Contains space
    EXPECT_FALSE(directive::validateHttpHost(string("192.168.1.1."))); // Ends with a dot

    // Test with consecutive dots
    EXPECT_FALSE(directive::validateHttpHost(string("192..168.1.1"))); // Contains consecutive dots

    // Test with leading or trailing dots
    EXPECT_FALSE(directive::validateHttpHost(string(".192.168.1.1"))); // Starts with a dot
    EXPECT_FALSE(directive::validateHttpHost(string("192.168.1.1."))); // Ends with a dot

    // Test with incorrect number of octets
    EXPECT_FALSE(directive::validateHttpHost(string("192.168.1"))); // Only 3 octets
    EXPECT_FALSE(directive::validateHttpHost(string("192.168.1.1.1"))); // 5 octets

    // Test with octets greater than 255
    EXPECT_FALSE(directive::validateHttpHost(string("256.168.1.1"))); // First octet > 255
    EXPECT_FALSE(directive::validateHttpHost(string("192.168.256.1"))); // Third octet > 255
    EXPECT_FALSE(directive::validateHttpHost(string("192.168.1.256"))); // Fourth octet > 255
...
TEST(DirectiveTest, ValidateHttpPort) {
    ...
    EXPECT_TRUE(directive::validateHttpPort(string("65535"))); // Maximum valid port
    ...
    EXPECT_FALSE(directive::validateHttpPort(string("8080a"))); // Contains non-numeric character
    ...
    EXPECT_FALSE(directive::validateHttpPort(string(" 8080 "))); // Leading/trailing spaces
    EXPECT_FALSE(directive::validateHttpPort(string("\t8080\t"))); // Leading/trailing tabs
}
...
TEST(DirectiveTest, AddListen) {
    ...
    EXPECT_NO_THROW(directive::addListen(string(""), listenList)); // Should do nothing
    EXPECT_EQ(listenList.size(), 3); // No change in size

    // Test with invalid listen string (invalid characters)
    EXPECT_THROW(directive::addListen(string("192.168.1.1:8080a"), listenList), runtime_error);

    // Test with invalid listen string (invalid host)
    EXPECT_THROW(directive::addListen(string("256.256.256.256:8080"), listenList), runtime_error);

    // Test with invalid listen string (invalid port)
    EXPECT_THROW(directive::addListen(string("192.168.1.1:65536"), listenList), runtime_error);

    // Test with duplicate listen string
    EXPECT_THROW(directive::addListen(string("192.168.1.1:8080"), listenList), runtime_error); // Already added
}

TEST(DirectiveTest, ValidateName) {
    // Test with valid names
    EXPECT_TRUE(directive::validateName(string("example"))); // Alphanumeric
    EXPECT_TRUE(directive::validateName(string("example123"))); // Alphanumeric with numbers
    EXPECT_TRUE(directive::validateName(string("example-name"))); // Alphanumeric with hyphen
    EXPECT_TRUE(directive::validateName(string("example.name"))); // Alphanumeric with dot
    ...
    EXPECT_FALSE(directive::validateName(string("-example"))); // Starts with hyphen
